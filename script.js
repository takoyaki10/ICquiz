const questions = [
    {
        ...
        correct: 3,
        explanation: "アイクラといえばポポとナナ 2人は友達以上恋人未満の関係だがスマブラDXでは恋愛関係がほのめかされている"
    },
    {
        question: "Q2 最もセンスのいいカラーは？",
        ...
        correct: 1,
        explanation: "問題の枠組みのデザインが青と水色を基調としていることから制作者にとってセンスのいいカラーは2Pだとわかる 忖度ではない"
    },
    {
        question: "Q3 次の選択肢のうち, あゆじゃん切り離し下投げ相方空前<br>を当てた際にファルコが立ち吹っ飛びになる最大の蓄積ダメージは？<br><br><b>条件 スマメイトルール1on1 アイクラ側はどちらも0% OPは初期状態<br>相方は大ジャンプ空前 回答は下投げ直前の%</b>",
        choices: ["17%", "18%", "19%", "20%"],
        correct: 0,
    ...
    },
    {
        ...
        choices: ["14", "15", "16", "17"],
        correct: 1,
        explanation: "選択画面では17番目だがダークサムスとデイジーがダッシュファイターのため15番目"
    },
    {
    ...
    {
        question: "Q8 スマブラSPにおいてアイクラの横強がアプデで変更されポポナックル&ナナナックル(通称ポックルナックル)になりました 1Pカラーの場合ポポが殴った後に同じ動きでナナが殴る技です 2人の攻撃がヒットした際の全体ダメージが70%のときポポの攻撃のダメージは何%でしょうか",
        choices: ["35%", "40%", "42%", "45%"],
        correct: 1,
        explanation: "相方の技のダメージは本体の0.75倍なので後は計算を頑張ってね NBとかは例外だよ"
    },
    {
    ...
    {
        question: "Q10 終点の両端にアイクラとファルコが向かい合いアイクラがNB(アイスショット)を使用します 地上でNBをした時と小ジャンプの頂点でNBをした時のファルコに与えるダメージについての記述で正しいのは？",
        choices: ["ちじょう > しょうジャンプ", "ちじょう < しょうジャンプ", "おなじ", "ランダム"],
        correct: 1,
        explanation: "NBは一定距離滑った後は地上を滑るほど大きさが小さくなりダメージも減ってしまう ジャンプNBだと地上を滑る距離が短くなるのでダメージが高いのだ"
    }
];
...
function showResult() {
    ...
    if(newscore == 50) document.getElementById("last-text").innerText = `いうことがない ホームランならすごい`;
    ...
    if(newscore == 20) document.getElementById("last-text").innerText = `だりつでもひくい めつぶってもおまえにかてる`;
